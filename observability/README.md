...
This directory captures the GeoSync observability stack as versioned code. All
...
- `metrics.json` – canonical catalogue of Prometheus metrics exported by
  GeoSync services. The file documents allowed labels, metric types, and the
  owning subsystem.
...
- `dashboards/` – Grafana dashboard definitions stored as JSON. Each file must
  ...
  - `geosync-production-operations.json` – Comprehensive production monitoring
    dashboard with system health, active alerts, SLO tracking, and key metrics.
  - `geosync-overview.json` – General system overview with order throughput
    and basic metrics.
  - `geosync-latency-insights.json` – Detailed latency analysis and breakdown.
  - `geosync-queue-operations.json` – Queue depth and processing metrics.
  - `geosync-resource-utilization.json` – CPU, memory, and disk utilization.
...
Kibana alongside the GeoSync services. Launch the stack with:
...
`geosync-logs-*` index pattern. Kubernetes users can rely on
