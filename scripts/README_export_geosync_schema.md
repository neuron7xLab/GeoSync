# export_geosync_schema.py

Export the GeoSync configuration JSON schema for validation and documentation.

## Description

This utility exports the JSON schema for GeoSync configuration settings. The schema can be used for:
...
```bash
python scripts/export_geosync_schema.py
```

### Export schema to file

```bash
python scripts/export_geosync_schema.py --output schemas/geosync-config.json
```

### Custom indentation

```bash
python scripts/export_geosync_schema.py --indent 4
```
...
The generated schema is a JSON Schema Draft 7 compliant document describing all GeoSync configuration options, including:
...
```bash
python scripts/export_geosync_schema.py --output .vscode/geosync-schema.json
```
...
```bash
# Export schema
python scripts/export_geosync_schema.py --output /tmp/schema.json
...
python scripts/export_geosync_schema.py --indent 4 > docs/config-reference.json
...
- Python 3.11+
- core.config module (GeoSync configuration system)
