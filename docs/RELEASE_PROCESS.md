# Release Process — GeoSync

> How to create releases — from merge to production.

## Overview

GeoSync використовує **trunk-based development**:
...
python -c "from importlib.metadata import version; print(version('geosync'))"
```
...
4. ✅ Push to `ghcr.io/neuron7xLab/GeoSync`
5. ✅ Sign images with Cosign

### Image Tagging:
```
ghcr.io/neuron7xLab/GeoSync:v1.2.3      # Version tag
ghcr.io/neuron7xLab/GeoSync:sha-abc123  # Commit SHA
ghcr.io/neuron7xLab/GeoSync:main        # Branch name
```
...
```bash
# Kubernetes
kubectl rollout undo deployment/geosync-api

# Docker
docker pull ghcr.io/neuron7xLab/GeoSync:v1.2.2  # Previous version
docker tag ghcr.io/neuron7xLab/GeoSync:v1.2.2 ghcr.io/neuron7xLab/GeoSync:latest
docker push ghcr.io/neuron7xLab/GeoSync:latest
```
...
cosign verify ghcr.io/neuron7xLab/GeoSync:v1.2.3 \
...
python -m geosync emergency-stop --reason "suspected issue"
```

### Rollback within 5 minutes
```bash
kubectl rollout undo deployment/geosync-api --to-revision=<previous>
```
