...
| --- | --- | --- | --- | --- |
| **Flash Crash 2010 (U.S. Equities)** | 2010-05-06 13:00–15:00 | Level II order-book + trade ticks for `/ES` and SPY. Snapshot to Parquet via [`geosync_cli ingest`](../cli/geosync_cli.py). | Queue depth deviation, fill latency percentiles, limit-order cancel ratio. | Execution simulator must trigger `HaltMode.PARTIAL` when order-book liquidity collapses, and risk controls clamp Kelly utilization to ≤50% after `max_drawdown` breaches 5%. |
