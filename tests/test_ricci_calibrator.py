...
from geosync.estimators.ricci_calibrator import RicciLeadTimeCalibrator


def test_random_data_remove_claim():
    """Random κ and returns → not predictive → REMOVE_CLAIM."""
    rng = np.random.default_rng(42)
    cal = RicciLeadTimeCalibrator()
...
def test_ci_ordered():
    ...
    if result.n_events > 0:
        assert result.lead_time_p5 <= result.lead_time_bars <= result.lead_time_p95
