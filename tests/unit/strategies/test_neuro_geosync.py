"""Unit tests for NeuroGeoSyncStrategy."""
...
def test_warmup_period_returns_zero_signals() -> None:
    ...
    warmup = 50
    cfg = NeuroGeoSyncConfig(warmup=warmup)
    bars = _sample_bars(100)
    strat = NeuroGeoSyncStrategy(cfg)
    signals = strat.generate_signals(bars)
...
def test_low_confidence_suppresses_signals() -> None:
    ...
    cfg = NeuroGeoSyncConfig(min_confidence=0.99, warmup=10)
    bars = _sample_bars(100)
    strat = NeuroGeoSyncStrategy(cfg)
    signals = strat.generate_signals(bars)
...
def test_negative_curvature_gate_suppresses_signals() -> None:
    ...
    cfg = NeuroGeoSyncConfig(negative_curvature_gate=1.0, warmup=10)
    bars = _sample_bars(100)
    strat = NeuroGeoSyncStrategy(cfg)
    signals = strat.generate_signals(bars)
...
def test_config_defaults() -> None:
    """NeuroGeoSyncConfig should have sensible defaults."""
    cfg = NeuroGeoSyncConfig()
