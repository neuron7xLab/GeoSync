...
def test_geosync_system_generates_features_and_orders(tmp_path: Path) -> None:
...
def test_geosync_system_rejects_hold_signal(tmp_path: Path) -> None:
    system = _build_system(tmp_path)
...
async def test_stream_market_data_uses_fake_async_ingestor(tmp_path: Path) -> None:
    ...
    system = GeoSyncSystem(config, async_data_ingestor=fake_ingestor)
