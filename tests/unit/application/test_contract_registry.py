...
def test_default_registry_contains_expected_contracts():
    ...
    signals_event = registry.get_event("geosync.events.signals")
    assert isinstance(signals_event, EventContract)
    assert signals_event.topic is EventTopic.SIGNALS

    queue_contract = registry.get_queue("geosync.queues.execution-requests")
...
def test_contract_registry_snapshot_is_stable():
    ...
    features = snapshot["api"]["geosync.api.v1.features"]
    predictions = snapshot["api"]["geosync.api.v1.predictions"]
    signals = snapshot["events"]["geosync.events.signals"]
    orders = snapshot["events"]["geosync.events.orders"]
    ...
    queue = snapshot["queues"]["geosync.queues.execution-requests"]
    assert queue["idempotency"]["ttl_seconds"] == 3600

    service = snapshot["services"]["geosync.service.execution.submit"]
