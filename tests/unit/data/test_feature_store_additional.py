...
class _SetWithTTLClient:
    ...
    def get(self, key: str) -> bytes | None:  # pragma: no cover - simple delegation
...
def sample_frame() -> pd.DataFrame:
    # The feature store contract guarantees ``datetime64[ns, UTC]``;
    # pandas 3 defaults the constructed Timestamp column to
    # ``datetime64[us, UTC]``, so cast at fixture creation.
    ...
    for row in frame.to_dict(orient="records"):
        ...
        for key, value in row.items():
            if isinstance(value, pd.Timestamp):
                encoded[key] = value.isoformat(timespec="nanoseconds").replace("+00:00", "Z")
            else:
                encoded[key] = value
...
def test_redis_rejects_non_tls_client() -> None:
    ...
    with pytest.raises(FeatureStoreConfigurationError):
        RedisOnlineFeatureStore(client=cast(KeyValueClient, _PlainRedisClient()))


def test_redis_tls_client_config(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_redis_tls_client_config_without_credentials(monkeypatch: pytest.MonkeyPatch) -> None:
    captured: dict[str, Any] = {}
...
def test_redis_tls_url_required(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_redis_client_tls_detection_handles_non_dict_pool() -> None:
    ...
    assert _redis_client_uses_tls(cast(KeyValueClient, _Client())) is True
...
def test_redis_online_store_rejects_conflicting_client_arguments() -> None:
    ...
    with pytest.raises(FeatureStoreConfigurationError):
        RedisOnlineFeatureStore(
            client=InMemoryKeyValueClient(),
            client_config=cast(RedisClientConfig, _StubConfig()),
        )
...
def test_format_numeric_value(value: Any, expected: str) -> None:
...
def test_sqlite_load_reapplies_retention_and_persists(
    ...
    def _capture_persist(feature_view: str, frame: pd.DataFrame) -> None:
