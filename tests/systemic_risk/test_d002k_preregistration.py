...
def test_dag_has_d002k_p0_and_d002j_p7_still_refused() -> None:
    dag = _load_json(DAG_VERDICT_JSON)
    msg_n = f"DAG must have 11 nodes; got {dag['nodes_count']!r}"
    assert dag["nodes_count"] == 11, msg_n
    msg_to = (
        f"D002K-P0 must be appended last in topological_order; got {dag['topological_order']!r}"
    )
    assert dag["topological_order"][-1] == "D002K-P0", msg_to
    ...
    msg_nl = f"next legal node must be D002K-P1; got {dag['next_legal_nodes_from_main_head']!r}"
    assert dag["next_legal_nodes_from_main_head"] == ["D002K-P1"], msg_nl
