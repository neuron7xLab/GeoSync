...
def _is_ancestor_of_main(sha: str) -> bool:
    ...
    Any other exit code (e.g. unknown ref) is surfaced via an assert
    so the test fails with context rather than silently returning False.
    """
...
def test_gate_g_preserves_d002c_ledger() -> None:
    ...
    consistency between artifact pin and disk sha.
    """
    payload = _load_payload()
    ...
    msg_disk = (
        ...
        f"got {actual_disk!r}; Gate G is forbidden from touching the D-002C "
        "claim ledger"
    )
